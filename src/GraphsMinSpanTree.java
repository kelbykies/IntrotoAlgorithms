 ...
 *         
 *  public static void minSpanTree(int start, int nodes,int edges, int [][] graph, int [][] spTable) 
 ...
 * @version 1
 */
public class GraphsMinSpanTree
{
    public static void minSpanTree(int start, int nodes, int edges, 
                                     int [][] graph, int [][] spTable)
    {
  
   // 1. initialize spTable   
   ...
   // 2. declare a priority queue of type Cell and size = number of edges
    	PQueueCell pQueue = new PQueueCell(edges);
    	
   // 3. insert the start node with priority 0 in the queue    
    	Cell temp = new Cell(0, start);	
	pQueue.insert(temp);

   
   /* 4. while queue is not empty   and counter > 0   
...
/*-------------------- print span tree ---------------------------*/
 public static void printSpanTree(int nodes, int [][] spTable)
 {
    ...
    for(int i = 0; i < nodes; i++)
    {
    		if(spTable[i][2] != 0)
    		{
    			System.out.println((i+1) + "-" + (spTable[i][1] + 1) + " weight " + spTable[i][2]);
    		}
    }
    System.out.println();
